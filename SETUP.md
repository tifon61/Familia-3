...
- **Editar quién sale en el mapa**: la lista de personas (nombre, avatar, bio) se edita en [`data/familiares.json`](data/familiares.json). La ubicación *actual* de cada uno, en cambio, no se edita ahí: se actualiza sola cuando esa persona hace "check-in" desde el formulario de la home (queda guardada en la hoja "Ubicaciones" de tu Google Sheet).
- **Troubleshooting "ID de archivo o carpeta no válido"**: pasa cuando `FOLDER_ID` en `Code.gs` no apunta a una carpeta real que tu cuenta (la que ejecuta el script) pueda ver. Revisá que copiaste solo el ID (sin `?usp=drive_link` ni el resto de la URL), que la carpeta pertenece a la misma cuenta de Google del script, y que volviste a implementar como "Nueva versión" después de cambiarlo.
